...
def test_time_stretch():
    ...
    test_time_stretch = librosa.effects.time_stretch(librosa.to_mono(mono_audio.raw_samples), stretch_amount)
...
def test_pitch_shift():
    ...
    test_pitch_shift = librosa.effects.pitch_shift(librosa.to_mono(mono_audio.raw_samples), mono_audio.sample_rate, shift_amount, bins_per_octave=step_size)
    test_pitch_shift_audio = Audio(raw_samples=test_pitch_shift, sample_rate=mono_audio.sample_rate)
    assert np.allclose(pitch_shift_audio.raw_samples, test_pitch_shift_audio.raw_samples, rtol=1e-3, atol=1e-4)
